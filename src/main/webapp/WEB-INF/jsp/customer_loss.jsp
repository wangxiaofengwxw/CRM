...
<script type="text/javascript">
		...
		$("#datagrid").datagrid({
			...
			columns:[[    
			     ...
			     {field:'a',title:'操作',width:80,align:'center',formatter:function(value,row,index){
			    	 if(row.status==0){
			    		 return "<a href='javascript:openDeferLossTab("+row.id+")'>暂缓流失</a>";
			    	 }else{
			    		 return "<a href='javascript:openAffirmLossTab("+row.id+")'>确认流失</a>";
			    	 }
			     }},    
			]]  
		});
	...
	function doSave() {
		$('#form').form('submit', {    
		    ...
		    onSubmit: function(){    
		        // do some check    
		        /* if($("#customerLossName").combobox("getValue") == "") {
	...
	//可以修改添加开发项
	function openDeferLossTab(id){
		 window.parent.openTab('暂缓流失措施'+id,'${ctx}/customerLoss/toLossMeasure.action?customerId='+id,'icon-khkfjh');
	}
	 
	//只能查看开发信息
	function openAffirmLossTab(id){
		window.parent.openTab('查看流失记录 '+id,'${ctx}/customerLoss/toLossMeasure.action?customerId='+id+'&show=true','icon-khkfjh');
	}
